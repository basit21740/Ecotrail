...
// Global variables for the timer and elapsed time
ALLEGRO_TIMER *timer = nullptr;
double elapsed_time = 0;

void startTimer() {
    timer = al_create_timer(1.0 / 60.0); // Create a timer that ticks at 60Hz
    al_start_timer(timer); // Start the timer
}

void updateTimer() {
    elapsed_time += al_get_timer_speed(timer); // Increment elapsed time by the timer speed
}

void displayTime(ALLEGRO_FONT *font) {
    al_draw_textf(font, al_map_rgb(255, 255, 255), 10, 10, 0, "Time: %.2f", elapsed_time);
}

void saveTimeToFile(const std::string &filename) {
